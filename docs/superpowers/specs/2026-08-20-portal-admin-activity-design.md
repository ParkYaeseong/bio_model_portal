...
- 기존 사용자(35명)는 다음 로그인 때 채워진다. 그 전까지는 `username` 을 **그대로** 보여
  준다. 축약하지 않는 이유는 두 가지다. UUID 를 8자로 줄여도 사람이 알아볼 수 없기는
  마찬가지이면서 값을 복사해 계정을 조회할 수 없게 되고, 실제 계정 29개 중 3개는 이미
  읽을 수 있는 이름(`sso:sp3-e2e-tester`)이라 축약이 오히려 훼손한다. 열 너비 조절은 UI 가
  담당한다. 계산된 표시명이 비었거나 접두어뿐이면 `(unknown)` 을 반환한다.
