...
from app import job_control as JC
...
def test_cancel_active_job_calls_client_and_marks_cancelled(monkeypatch):
    ...
    _FakeClient.calls = []
    monkeypatch.setattr(JC, "RunpodClient", lambda: _FakeClient())
...
def test_cancel_survives_remote_failure(monkeypatch):
    ...
    monkeypatch.setattr(JC, "RunpodClient", lambda: _Boom())
    with SessionLocal() as db:
...
def test_mcp_cancel_stops_the_compute_too(monkeypatch):
    """Cancelling over MCP has to free the GPU, not just flip the row —
    otherwise a cancelled AlphaFold3 run keeps the card for another hour."""
    import uuid
    ...
    monkeypatch.setattr(JC, "RunpodClient", lambda: _FakeClient())
    with SessionLocal() as db:
        user = models.User(username=f"c_{uuid.uuid4().hex[:8]}", password_hash="x")
        db.add(user); db.commit(); db.refresh(user)
        job = _job(db, user.id, "in_progress")
        out = tools.cancel_job(db, user, {"job_id": job.id})
        assert out["ok"] is True and out["status"] == "cancelled"
        assert _FakeClient.calls == [("ep1", "gw-1")]
