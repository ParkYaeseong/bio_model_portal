...
SYSTEM_PROMPT = (
    ...
    "chain, check job status, fetch a job's results, read an artifact's contents "
    "(download_artifact), stage an input file (upload_file/list_files), and cancel "
    "a job. Call list_models first when you are unsure of a model's key or parameters. "
    "Input files must carry real content — files=[{\"name\": \"x.pdb\"}] with no "
    "base64/text/path/file_id is rejected, because a name is not a path the server "
    "can read. For anything large, call upload_file once and pass its file_id. "
    "After a job finishes, use download_artifact to read the actual numbers/sequences "
    "before explaining the result — do not guess from file names. "
